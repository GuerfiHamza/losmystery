...
@section('content')
    ...
    <h3 class="font-bebas text-5xl mt-2 tracking-wide text-white leading-tight">To Serve</h3>
    ...
    <a href="{{ route('ems-archives.create') }}" class="btn-primary transition duration-300 ease-in-out focus:outline-none focus:shadow-outline bg-yellow-600 hover:bg-yellow-900 text-white font-normal py-2 px-4 mr-1 rounded-2xl font-opensans">Créer un dossier</a>

    
    <div class="w-2/3 mx-auto bg-gray-900 text-white p-2 rounded font-opensans">
        <table id="archives-table" class="bg-gray-900 text-white font-opensans">
            <thead class="bg-gray-900 text-white">
                <tr class="bg-gray-900 text-white">
                    <th class="bg-gray-900 text-white">Nom</th>
                    <th class="bg-gray-900 text-white">Medecin</th>
                    <th class="bg-gray-900 text-white">Amende</th>
                    <th class="bg-gray-900 text-white">Numero de tel</th>
                    <th class="bg-gray-900 text-white">Mort</th>
                    <th class="bg-gray-900 text-white">PPA</th>
                </tr>
            </thead>
            <tbody>
                @foreach($archives as $archive)
                    <tr>
                        <td><a href="{{ route('ems-archives.show', compact('archive')) }}">{{ $archive->patient->name }}</a></td>
                        <td>{{ $archive->who }}</td>
                        <td>{{ $archive->amende }} $</td>
                        <td>{{ $archive->patient->tel }}</td>
                        <td>{{ $archive->patient->dead ? "Mort" : "Non" }}</td>
                        <td>{{ $archive->ppa ? "Oui" : "Non" }}</td>
